...
Problem Statement:
	It is difficult for the faculties to draw/write the iterations again and again on the board every time and it is so lame to use ppts as it is not that much interactive with the students and the faculties are able to add max. 2 examples for each topic.

The Proposal:
	An app that helps the faculties to teach/demonstrate algorithms step-by-step with very subtle animations, showing how the algorithms actually works during every step from taking inputs,going through each step/iterations and then finally the output.
